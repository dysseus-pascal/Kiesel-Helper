...
/**
 * Die Lagen des Widgets - auf Deutsch, mit fester Locale.
 *
 * FEST, WEIL DIE ZAHLEN DER SPRACHE FOLGEN: "4.000" auf Deutsch, "4,000" auf
 * Englisch. Ohne feste Locale hinge das Ergebnis davon ab, auf welchem
 * Rechner der Test laeuft.
 */
class WidgetlageTest {

    private lateinit var vorher: Locale
    private val texte = TestTexte("values-de", Locale.GERMAN)
    ...
    @After
    fun zurueck() = Locale.setDefault(vorher)

    private fun ermittle(b: Widgetlage.Blick) = Widgetlage.ermittle(b, texte)

    companion object {
        /** Ein Stand zum Pruefen; auch der Sprachentest braucht ihn. */
        fun blick(
    ...
    fun trainingSchlaegtMorgen() {
        ...
        assertTrue(l.satz, l.satz.contains("7,2 km"))
    }
    ...
    fun offenePraeparateErinnern() {
        val l = ermittle(blick(offene = listOf("Magnesium", "Zink", "D3")))
    ...
    fun wasserImRueckstandErinnert() {
        ...
        val l = ermittle(blick(jetzt = LocalDateTime.of(2026, 9, 24, 15, 0), wasserMl = 300.0))
        assertEquals(Widgetlage.Art.ERINNERUNG, l.art)
    ...
    fun nachmittagsKeinMahnenWennDerMorgenReichte() {
        ...
        val l = ermittle(blick(jetzt = LocalDateTime.of(2026, 9, 24, 16, 0), wasserMl = 1200.0))
        assertEquals(Widgetlage.Art.TAG, l.art)
    }

    @Test
    fun abendsDieBilanz() {
        val l = ermittle(blick(jetzt = LocalDateTime.of(2026, 9, 24, 21, 0), wasserMl = 2100.0))
        assertEquals(Widgetlage.Art.ABEND, l.art)
        assertTrue(l.satz, l.satz.contains("noch 4.000 Schritte"))
        assertTrue(l.satz, l.satz.contains("1 Glas fehlt"))
    }

    @Test
    fun derWunschUebersteuert() {
        val l = ermittle(blick(wunsch = Widgetlage.Art.MORGEN))
        assertEquals(Widgetlage.Art.MORGEN, l.art)
    }
}
